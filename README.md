...
3. 添加配置信息，以下为写法示例
   ...
   electric_clock:
     ...
     default_province: 福建省 # 获取访问者位置失败时使用的省
     default_city: 福州市 # 获取访问者位置失败时使用的市
     default_county: 闽侯县 # 获取访问者位置失败时使用的县
   ```

4. 参数释义

说明：纯静态站点默认可用，天气数据来自`https://uapis.cn/api/v1/misc/weather`，定位数据来自`https://uapis.cn/api/v1/network/myip`。定位失败时使用默认省市县；天气接口不可用时将不显示天气数据。

天气接口参数（uapis）：

- city: 标准城市名称（如“北京”“上海市”“福田区”），默认使用省/市/区县回退链。
- adcode: 6位数字城市编码（更准确）。
- extended: 是否返回扩展气象字段。
- indices: 是否返回生活指数。
- forecast: 是否返回预报数据。
...
| default_province | 【可选】获取访问者位置失败时使用的省 |
| default_city     | 【可选】获取访问者位置失败时使用的市 |
| default_county   | 【可选】获取访问者位置失败时使用的县 |
