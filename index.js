...
hexo.extend.filter.register('after_generate', function (locals) {
  ...
  const data = {
    ...
    clock_js: config.clock_js ? urlFor(config.clock_js) : "https://cdn.cbd.int/hexo-butterfly-clock-anzhiyu/lib/clock.js",
    default_province: config.default_province ? config.default_province : "福建省",
    default_city: config.default_city ? config.default_city : "福州市",
    default_county: config.default_county ? config.default_county : "闽侯县",
  }
  // 渲染页面
  ...
  var flag = 0;
  var clock_default_province = '${data.default_province}';
  var clock_default_city = '${data.default_city}';
  var clock_default_county = '${data.default_county}';
