...
function getIpInfo() {
  ...
  function normalizeLocation(data, fallback) {
    ...
    var province = data && data.province ? data.province : regionInfo.province
    var city = data && data.city ? data.city : regionInfo.city
    var county = data && (data.district || data.county) ? (data.district || data.county) : fallback.county
    return {
      province: province || fallback.province,
      city: city || fallback.city,
      county: county,
      displayCity: city || fallback.city
    }
  }

  function buildWeatherUrl(location) {
    var cityValue = location.county || location.city || location.province
    var params = new URLSearchParams({
      city: cityValue
    })
    return `https://uapis.cn/api/v1/misc/weather?${params.toString()}`
  }
  ...
  function normalizeWeatherResponse(data) {
    ...
    if (data.data && data.data.observe) {
      return data.data.observe
    }
    if (data.weather || data.temperature || data.humidity) {
      return {
        weather: data.weather,
        degree: data.temperature,
        humidity: data.humidity,
        wind_direction_name: data.wind_direction,
        wind_direction: data.wind_direction,
        weather_code: data.weather_code,
        weather_url: data.weather_url
      }
    }
    return null
  }
  ...
  function fetchWeatherAndRender(location) {
    return fetch(buildWeatherUrl(location))
      .then(res => res.json())
      .then(data => {
        var observe = normalizeWeatherResponse(data)
        if (!observe) {
          return
        }
        if (document.getElementById('hexo_electric_clock')) {
          var displayCity = (data && data.city) || location.displayCity || location.city || location.province
          clockUpdateTime(observe, displayCity)
        }
      })
  }

  var fallbackLocation = buildFallbackLocation()
  var ipParams = new URLSearchParams({ source: 'commercial' })
  var ipUrl = `https://uapis.cn/api/v1/network/myip?${ipParams.toString()}`
  fetch(ipUrl)
    .then(res => res.json())
